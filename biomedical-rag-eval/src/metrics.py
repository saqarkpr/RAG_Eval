...
def bootstrap_ci(values, n_boot: int = 1000, seed: int = 42):
    """Bootstrapped 95% CI on the mean of a per-item binary/real-valued
    metric array (e.g. per-query recall-hit, per-query correctness).
    Shared by retrieval and generation eval so both report variance the
    same way rather than a single point estimate on ~200 test items.
    """
...
_STOPWORDS = {
    ...
    "has", "are", "but", "our", "these", "those", "than", "into", "such",
    "may", "can", "also", "been", "did", "does", "each", "any", "all", "more",
}
# "not" is deliberately NOT a stopword here: "not effective" and "effective"
# have opposite clinical meaning, and a lexical overlap metric that discards
# negation would call a claim "supported" by evidence that actually negates
# it. This does not turn support_ratio into a real negation/entailment
# check (see its docstring) -- it just stops the metric from throwing away
# the one word most likely to flip a sentence's meaning.
