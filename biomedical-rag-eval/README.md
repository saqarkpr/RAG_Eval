...
1. Prompt the model with the retrieved evidence and ask for a structured
   `{decision, confidence, quoted_evidence}` response.
2. Parse that into a `GenerationResult`.
3. Re-run `scripts/run_generation_eval.py` unmodified — accuracy, ECE,
   Brier, and support-ratio numbers above are then real numbers for a real
   LLM, comparable apples-to-apples against the baseline in this README
   because the eval code didn't change, only the generator did.
...
src/
  ...
  generator.py    ExtractiveClassifierGenerator, HFLocalLLMGenerator (stub)
